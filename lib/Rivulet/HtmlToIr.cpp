...
bool appendCollapsedText(std::string& dst, const char* p, size_t n, bool preserveSpace) {
  ...
  while (s < end) {
    ...
    if (c < 0x80) {
      ...
      if (c == '\n' || c == '\t' || c == ' ') {
        if (!preserveSpace) {
          // Collapse runs of whitespace to a single space, but never drop a
          // leading space when dst is empty. That space is often the inter-word
          // gap after a style boundary (</span> word) — stripping it glued
          // "newcomers"+"stopped". Block-start leading spaces are stripped in
          // flushText when firstRun is true.
