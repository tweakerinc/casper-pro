...
Casper keeps CrossPoint's stable reader core, and CrossInk's AMAZING Stat Tracking, and adds a redesign to the UI ΓÇö thoughtful long-press menus, fully customizable **Reader UI**, **Synopsis Viewer**, and **StarDict Dictionary** with multi-word selection and bilingual packs. Home themes: **Penumbra** (X3 clock / X4 title) and **Bare**.
...
- Long-press **Read** ΓåÆ book quick menu (mark finished, remove from recents, clear cache, synopsis, etc.)

#### Bare

Designed for readers who simply want to feel as if it is them and their book ΓÇö a minimal cover theme, same layout on X3 and X4.
...
Keyboard / WiΓÇæFi password footer labels follow your remap (Left stays Left, Up stays Up).
...
You can turn stats off completely if you prefer a distraction-free reading experience. When tracking is on, PenumbraΓÇÖs Book Stats and Lifetime panels use that data.
...
Open a short description of the current book from the **book quick menu** (long-press **Read** on home, or long-press a book in the library). Requires synopsis metadata ΓÇö easy to add with Calibre.
...
Returning to the home cover after reading should feel faster and less ΓÇ£busy.ΓÇ¥
...
- If cover thumbnails are already on disk (the usual case after youΓÇÖve opened a book before), Home skips the intermediate black-and-white shell refresh and goes straight to the grayscale cover pass.

#### Fewer wasted full redraws

- Cancelling the book action menu (long-press Read ΓåÆ Back/cancel) repaints Home once and no longer forces a full cover re-scan/generation.
...
| Boot | After flash / cold boot ΓåÆ **Home**, not last book. Quick Resume still works on sleep wake (including X4 on battery). |
| Bare menu | **Back** closes the menu and restores the cover. |
| WiΓÇæFi keyboard | Footer labels match button remapping (front + side). |
| Penumbra | Weekday more reliable after clock sync. |
| Notes | One bin for X3 and X4. WiΓÇæFi passwords live on SD (`/.crosspoint/wifi.json`, device-tied). Sync clock on X3 if weekday/time look wrong after install. |
