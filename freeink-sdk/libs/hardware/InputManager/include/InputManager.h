...
public:
  ...
  // Re-run touch controller bring-up (power rail + I2C probe). Used after late
  // board rail asserts (X4 Pro GPIO1 master rail) when the first probe failed.
  void reinitTouch() { beginTouch(); }
  // Runtime probe result (true only if the controller ACKed). Distinct from
  // BoardConfig::hasTouch() which is compile-time profile capability.
  bool touchControllerPresent() const { return touchDataEnabled; }
...
private:
  ...
  static constexpr unsigned long TOUCH_LONG_PRESS_MS =
      700; // stationary hold for dictionary/highlight (swipe page-turn must not
           // fire this; 500ms was too easy to trip on a slow swipe start)
