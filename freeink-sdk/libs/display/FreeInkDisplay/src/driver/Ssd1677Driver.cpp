...
// Xteink X4 Pro (ESP32-S3). Same glass class as the C3 X4, but OEM app1 uses
// different Display Update Control 2 (0x22) sequences recovered from the dump
// and confirmed in freeink-sdk/docs/xteink-x4pro-support.md:
//   FULL = 0xF7, FAST = 0xC7
// Using the C3 X4 fast sequence (0xFC) on Pro makes MASTER_ACTIVATION a no-op
// on many units: BUSY never asserts, waitRefreshComplete returns in ~20 ms, and
// the glass never changes — only peripherals like the frontlight appear alive.
static const Ssd1677Config& ssd1677X4ProConfig() {
  static const Ssd1677Config cfg = {
      {0xAE, 0xC7, 0xC3, 0xC0, 0x80},  // booster soft-start (OEM INIT)
      DRIVER_OUTPUT_SCAN,
      0x80,  // borderWaveformInit: OEM INIT border 0x3C=0x80
      0x5A,  // halfRefreshTemp
      lut_grayscale,
      0xF7,  // fullSeqOverride: OEM FULL
      0xC7,  // fastSeqOverride: OEM FAST (NOT X4's 0xFC)
      0xF7,  // halfSeqOverride: clean full waveform (no OEM HALF documented)
      0x80,  // borderWaveformFull
      0x80,  // borderWaveformFast
      0x80,  // borderWaveformHalf
      0x80,  // borderWaveformGray
  };
  return cfg;
}
...
void Ssd1677Driver::displayImpl(EpdBus& bus, const uint8_t* fb, const uint8_t* prev, RefreshMode mode, bool turnOff,
  ...
  // X4 Pro FAST 0xC7 / FULL 0xF7 (and Sticky 0xFF) self-power-down after the
  // waveform (seq low bits 0x03). RAM writes while the controller is off are
  // dropped — the host advances the page, but the glass never changes until a
  // later absolute scrub. Clock/analog must be up before every writeRam.
  // X4 C3 FAST 0xFC has no power-off bits, so _isScreenOn stays true and this
  // is a no-op between page turns.
  ...
  const auto writePlanes = [&]() {
    ...
    // single-buffer mode (prev == nullptr) the RED "old" plane is then not a
    // trustworthy previous frame — differential FAST becomes a no-op: the
    // host advances the page, the glass stays put until a FULL scrub.
    // Force every pixel to redrive by feeding RED = ~target (same as dark-
    // background). Dual-buffer still has a real prev and keeps true DU.
    const bool powerDownSeq = (_cfg.fastSeqOverride & 0x03) != 0;
    if (_darkBackground || (prev == nullptr && powerDownSeq)) {
  ...
  //
  // Sequences that self-power-down leave the controller off here — power up
  // first or the baseline write is a no-op and the next FAST diffs against
  // garbage (page advances in software, glass stuck).
...
void Ssd1677Driver::displayWindow(EpdBus& bus, const uint8_t* fb, const uint8_t* prev, uint16_t x, uint16_t y,
  ...
  // Same Pro/Sticky self-power-down caveat as displayImpl — wake before RAM writes.
  if (!_isScreenOn && !turnOff) {
    powerOn(bus);
  }
...
static const Ssd1677Config& ssd1677ActiveConfig() {
  switch (BoardConfig::ACTIVE.board) {
    ...
    // X4 Pro: OEM 0x22 sequences (FULL 0xF7 / FAST 0xC7). Do not reuse C3 X4 0xFC.
