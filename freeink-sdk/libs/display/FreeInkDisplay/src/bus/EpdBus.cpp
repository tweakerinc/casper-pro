...
// Cap BUSY waits. 30s made every Pro paint feel bricked when the pin floated
// HIGH. Full OTP waveforms finish in ~1–2s; 5s is generous.
static constexpr unsigned long BUSY_WAIT_MAX_MS = 5000;
...
void EpdBus::waitBusy(BusyPolarity p, const char* tag) {
  ...
  if (p == BusyPolarity::ActiveHigh) {
    while (digitalRead(_pins.busy) == HIGH) {
      ...
      if (millis() - start > BUSY_WAIT_MAX_MS) break;
    }
...
void EpdBus::waitRefreshComplete(const char* tag) {
  ...
  while (digitalRead(_pins.busy) != doneLevel) {
    if (xSemaphoreTake(s_epdRefreshDone, pdMS_TO_TICKS(BUSY_WAIT_MAX_MS)) != pdTRUE) break;
  }
