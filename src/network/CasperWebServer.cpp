...
// Dot-names are hidden separately; XTCache is stock Xteink (not Casper — we use /.crosspoint).
...
bool isProtectedItemName(const String& name) {
  ...
  return SdHiddenNames::isAlwaysHidden(name.c_str());
}
}  // namespace
...
void CasperWebServer::scanFiles(const char* path, const std::function<void(FileInfo)>& callback) const {
  ...
  while (file) {
    ...
    if (!shouldHide && SdHiddenNames::isAlwaysHidden(fileName.c_str())) {
      shouldHide = true;
    }
...
void CasperWebServer::handleDownload() const {
  ...
  if (SdHiddenNames::isAlwaysHidden(itemName.c_str())) {
