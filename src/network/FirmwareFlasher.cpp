...
uint16_t runningPartitionChipId() {
  // esp_partition_read hits SPI flash; cache once per boot. Running image is
  // immutable at runtime, so a function-local static is safe.
  static uint16_t cached = [] {
    const esp_partition_t* run = esp_ota_get_running_partition();
    if (!run) return static_cast<uint16_t>(0xFFFF);
    uint16_t id = 0xFFFF;
    // chip_id at offset 12 of esp_image_header_t.
...
Result validateImageFile(const char* sdPath, size_t partitionSize) {
  ...
  // device (e.g. Sticky ESP32-S3 bin on X3/X4 C3). chip_id @ header offset 12.
  uint16_t imageChip;
