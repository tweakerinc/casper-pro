...
void WebDAVHandler::handlePropfind(WebServer& s) {
  ...
  if (depth > 0) {
    ...
    while (file) {
      ...
      // Skip hidden/protected items (stock XTCache, SVI, etc.)
      bool shouldHide = fileName.startsWith(".") || SdHiddenNames::isAlwaysHidden(fileName.c_str());
...
bool WebDAVHandler::isProtectedPath(const String& path) const {
  ...
  while (start < (int)path.length()) {
    ...
    if (segment.startsWith(".") || SdHiddenNames::isAlwaysHidden(segment.c_str())) return true;
